...
console.log('[TickS Terrain] v1.4.1 — prêt');
...
const LABELS = { entree:'Entree batiment', equip_comp:'Info voyageur', equip_acces:'Equip. acces', noeud:'Noeud cheminement', autre:'Autre' };
const SUBS = {
...
function launchWatch(highAccuracy){
  ...
  GPS_WATCH_ID=navigator.geolocation.watchPosition(
    ...
    err=>{
      const bar=document.getElementById('gps-bar-txt');
      if(bar){bar.textContent='Err';bar.style.color='var(--red,#FF3B30)';}
      if(err.code===1)toast(/iPhone|iPad|iPod/.test(navigator.userAgent)?'Autorisez la localisation dans Reglages Safari':'Permission GPS refusee','r');
      else if(err.code===3)toast('GPS timeout — allez a l exterieur','a');
    },
...
function initMap(){
  if(!MAP_OK){
    ...
    MAP.setView([49.18,0.35],9);MAP_OK=true;
...
function startCapture(type){
  ...
  if(t)t.textContent='Leve '+typeLabel(type);
...
function typeLabel(t){return {entree:'Entree',equip_comp:'Info voy.',equip_acces:'Equip.acces',noeud:'Noeud',autre:'Autre'}[t]||t;}
...
function renderPts(){
  ...
  if(!all.length){el.innerHTML='<div class="empty">Aucun point enregistre.<br>Allez dans Terrain pour commencer.</div>';return;}
...
function pickType(t,el){document.querySelectorAll('#em-types .topt').forEach(e=>e.classList.remove('sel'));el.classList.add('sel');document.getElementById('em-sub').innerHTML=(SUBS[t]||['AUTRE']).map(v=>'<option>'+v+'</option>').join('');}
...
function updateTrkStats(){
  ...
  if(ep)ep.textContent=t.pts.length;if(ed)ed.textContent=d;
}
function updateSheetUI(){
  ...
  if(S.recording){
    if(nm)nm.value=S.curTrack!==null?S.tracks[S.curTrack].name:'Troncon';
    ...
    if(btn){btn.innerHTML='<span>\u25cf</span><span> Enregistrer un troncon</span>';btn.style.background='';btn.style.color='';}
...
let WAKE_LOCK=null;
async function requestWakeLock(){if(!('wakeLock' in navigator))return;try{WAKE_LOCK=await navigator.wakeLock.request('screen');WAKE_LOCK.addEventListener('release',()=>{WAKE_LOCK=null;});}catch(e){}}
...
window.addEventListener('unhandledrejection',e=>{
  const msg=e.reason&&e.reason.message||String(e.reason)||'Promesse rejetee';
  console.error('[TickS] Async:',msg);
  if(typeof toast==='function')toast('\u26a0 '+msg.slice(0,60),'r');
});

// Version visible dans le burger
...
function closeFilterPopover(){
  const p=document.getElementById('filter-popover'),b=document.getElementById('btn-filter-toggle');
  if(p)p.classList.remove('open');if(b)b.classList.remove('active');
}
